...
export const useShop = defineStore('shop', {
  state: () => ({
    ...
    active: 'dashboard',
    whatsappSettings: {
      phone_number_id: localStorage.getItem('alpha-wa-phone-id') || '',
      whatsapp_token: localStorage.getItem('alpha-wa-token') || '',
      verify_token: localStorage.getItem('alpha-wa-verify-token') || 'alphashop_whatsapp_verify_token_123',
      ai_provider: localStorage.getItem('alpha-wa-ai-provider') || 'openai',
      ai_api_key: localStorage.getItem('alpha-wa-ai-key') || '',
      ai_model: localStorage.getItem('alpha-wa-ai-model') || 'gpt-4o-mini',
      system_prompt: localStorage.getItem('alpha-wa-sys-prompt') || 'أنت مساعد مبيعات احترافي لمتجرنا في المغرب. تجيب الزبائن بالدارجة المغربية أو الفرنسية أو العربية بأسلوب مؤدب وسريع. تساعدهم في معرفة تفاصيل المنتجات، الأسعار، والتوصيل، وتشجعهم على تقديم طلباتهم.',
      auto_reply_enabled: localStorage.getItem('alpha-wa-auto-reply') !== 'false',
      include_products_context: localStorage.getItem('alpha-wa-products-ctx') !== 'false'
    },
    whatsappChats: []
  }),
  ...
  actions: {
      ...
      if (supabase) {
        try {
          ...
          if (data && !error) {
            ...
            localStorage.setItem('alpha-wa-token', data.whatsapp_token || '')
            localStorage.setItem('alpha-wa-verify-token', data.verify_token || '')
            localStorage.setItem('alpha-wa-ai-provider', data.ai_provider || 'openai')
            localStorage.setItem('alpha-wa-ai-key', data.ai_api_key || '')
            localStorage.setItem('alpha-wa-ai-model', data.ai_model || 'gpt-4o-mini')
            localStorage.setItem('alpha-wa-sys-prompt', data.system_prompt || '')
            localStorage.setItem('alpha-wa-auto-reply', data.auto_reply_enabled ? 'true' : 'false')
            localStorage.setItem('alpha-wa-products-ctx', data.include_products_context ? 'true' : 'false')
          }
        } catch (e) {
          console.warn('Unable to load WhatsApp settings from Supabase:', e.message)
        }
      }
    },

    async saveWhatsAppSettings(settings) {
      this.whatsappSettings = { ...this.whatsappSettings, ...settings }
      localStorage.setItem('alpha-wa-phone-id', settings.phone_number_id || '')
      localStorage.setItem('alpha-wa-token', settings.whatsapp_token || '')
      localStorage.setItem('alpha-wa-verify-token', settings.verify_token || '')
      localStorage.setItem('alpha-wa-ai-provider', settings.ai_provider || 'openai')
      localStorage.setItem('alpha-wa-ai-key', settings.ai_api_key || '')
      localStorage.setItem('alpha-wa-ai-model', settings.ai_model || 'gpt-4o-mini')
      localStorage.setItem('alpha-wa-sys-prompt', settings.system_prompt || '')
      localStorage.setItem('alpha-wa-auto-reply', settings.auto_reply_enabled ? 'true' : 'false')
      localStorage.setItem('alpha-wa-products-ctx', settings.include_products_context ? 'true' : 'false')
      ...
      if (supabase) {
        try {
          ...
          this.notify('Éléments de WhatsApp enregistrés ✓')
        } catch (err) {
          console.error('Error saving WhatsApp settings to Supabase:', err)
          this.notify('Enregistré en local ✓ (Base Supabase indisponible)')
        }
      } else {
        this.notify('Paramètres enregistrés en local ✓')
      }
    },
    ...
    async fetchWhatsAppChats() {
      ...
      try {
        const { data, error } = await supabase
          ...
          .limit(50)
        if (data && !error) {
          this.whatsappChats = data
        }
      } catch (err) {
        console.warn('Failed to fetch WhatsApp chats:', err.message)
      }
    },

    async sendManualWhatsAppMessage(phoneNumber, message) {
      try {
        const res = await fetch('/api/whatsapp-webhook', {
          ...
          body: JSON.stringify({
            ...
            phoneNumber,
            message,
            settings: this.whatsappSettings
          })
        })
        const data = await res.json()
        if (!res.ok || data.error) throw new Error(data.error || 'Erreur lors de l\'envoi de la réponse WhatsApp')
        this.notify('Message WhatsApp envoyé avec succès ✓')
    ...
    async testAiResponse(messageText) {
      const activeProducts = (this.products || []).map(p => ({
        name: p.name,
        price: p.price,
